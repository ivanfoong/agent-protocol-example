...
async def task_handler(task: Task) -> None:
    ...
    print(task_input)
    for i in range(task_input['count_to']):
        print('created step', i)
        step_input = { "count_to": task_input['count_to'] }
        await Agent.db.create_step(task.task_id, name="count", input=json.dumps(step_input), is_last=i==task_input['count_to']-1)
...
async def step_handler(step: Step) -> Step:
    ...
    if step.name == 'count':
        print("step.input", step.input)
        input = json.loads(step.input)
        print(input['count'])
